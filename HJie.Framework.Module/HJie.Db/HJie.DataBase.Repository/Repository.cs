    ...
    public class Repository : IRepository
    {
        ...
        public T FindEntity<T>(string strSql, object dbParameter) where T : class, new()
        {
            if (dbWhere != null)
            {
                ...
                if (orderIndex > 0)
                {
                    ...
                    if (1 == dbWhere.isrecursion)
                    {
                        strSql = string.Format(" select distinct * From ({0})t  start with {1} {2} {3} ", strSql, dbWhere.sql, dbWhere.recursionsql, orderString);
                    }
                    else
                    {
                ...
                else
                {
                    if (1 == dbWhere.isrecursion)
                    {
                        strSql = string.Format(" select distinct * From ({0})t  start with {1} {2} ", strSql, dbWhere.sql, dbWhere.recursionsql);
                    }
                    else
                    {
        ...
        public IEnumerable<T> FindList<T>(string strSql, object dbParameter, Pagination pagination) where T : class
        {
            ...
            if (dbWhere != null)
            {
                ...
                DynamicParameters dynamicParameters = SqlHelper.FieldValueParamToParameter(dbWhere.dbParameters);
                dynamicParameters.AddDynamicParams(dbParameter);
                System.Diagnostics.Debug.WriteLine(System.DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss  fff ") + " Begin db.FindList : " + strSql, "hx");
                var data = db.FindList<T>(strSql, dynamicParameters, pagination.sidx, pagination.sord.ToLower() == "asc" ? true : false, pagination.rows, pagination.page, out total);
                System.Diagnostics.Debug.WriteLine(System.DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss  fff ") + " End db.FindList : " + strSql, "hx");
                pagination.records = total;
                return data;
            }
            else
